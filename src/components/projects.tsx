...
export function Projects() {
  return (
    <Section
      ...
      description="A closer look at the platforms I've shipped — from fundraising technology to healthcare, learning and events."
    >
  ...
  return (
    <SpotlightCard className="h-full rounded-3xl">
      <motion.article
        ...
        className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-surface transition-colors duration-300 hover:border-accent/50"
      >
        {/* Soft colour glow that blooms on hover */}
        <div
          aria-hidden
          className="pointer-events-none absolute -inset-px opacity-0 blur-2xl transition-opacity duration-500 group-hover:opacity-30"
          style={{ background: `linear-gradient(135deg, ${from}, ${to})` }}
        />

        {/* ── Image area ─────────────────────────────────────── */}
        {/* The tinted background doubles as the skeleton while the image loads */}
        <div
        ...
        >
          {p.image ? (
            <Image
              ...
              fill
              /* Cards cap out at ~468px (max-w-5xl grid, 2 cols, gap-6) — telling
                 the browser that keeps it from pulling a 1920w variant. */
              sizes="(min-width: 1024px) 468px, (min-width: 768px) 50vw, 100vw"
              className="object-cover transition-transform duration-700 ease-out group-hover:scale-[1.04]"
            />
          ) : (
            <ImagePlaceholder title={p.title} from={from} to={to} />
          )}

          <span className="absolute left-4 top-4 rounded-full border border-white/25 bg-black/35 px-2.5 py-1 text-[11px] font-medium text-white backdrop-blur-md">
            {p.tagline}
          </span>
          {p.year && (
            <span className="absolute right-4 top-4 rounded-full border border-white/25 bg-black/35 px-2.5 py-1 text-[11px] font-medium text-white backdrop-blur-md">
              {p.year}
            </span>
          )}
...
/**
 * Circular icon button revealed on card hover. Scales up + fades in with a
 * small stagger, and shows a label tooltip on its own hover.
 */
...
/** Stand-in artwork shown until a real screenshot is dropped in. */
function ImagePlaceholder({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="absolute -bottom-10 -right-10 h-40 w-40 rounded-full blur-3xl transition-transform duration-700 group-hover:scale-125"
        style={{ background: `linear-gradient(135deg, ${from}, ${to})`, opacity: 0.35 }}
      />
      <div className="relative flex flex-col items-center gap-2 text-center">
        ...
        <span className="text-xs font-medium text-muted">{title} preview</span>
      </div>
    </div>
  );
}
